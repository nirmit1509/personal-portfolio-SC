...
ETH contract (TODOs):
1. set bridge fee address.
2. set charity wallet address.

After deploying contract
3. set isBridgingEnabled to 'true'
4. set vaultAddress
5. set system address
6. exclude accounts - pair, bridgeAddress, vaultAddress

Vault:
1. transfer ownership to eth contract address.
...
contract Catoshi is IERC20, Ownable {
  ...
  function setFTMVaultaddress(address _add) public onlyOwner returns (bool) {
    require(_add != address(0), 'Invalid Address');
    _ftmTokenVault = vault(_add);
    return true;
  }
  ...
  function swap(uint256 amount) external payable {
    ...
    _nonce = _nonce.add(1);
    _transfer(_msgSender(), address(_ftmTokenVault), amount);
  ...
  function swapBack(
    ...
    _ftmTokenVault.transferamount(temp, to, _cats);
    _ftmTokenVault.transferamount(bridgeFees, _bridgeFeesAddress, _cats);
    nonceProcessed[nonce] = true;
  }

  function withdrawFTM(uint256 amount, address receiver) external onlyOwner {
