...
contract MyLotto is IERC20 {
  ...
  constructor(
    ...
    address buyerPoolAddress,
    address owner
  ) {
    _name = n;
    _symbol = s;
    _decimals = d;
    _totalSupply = t * (10**uint256(d));
   ...
   */
  function airdropToken(address[] memory _addresses, uint256[] memory _amount)
    public
    onlyOwner
    returns (bool)
  {
  ...
  function airdropEther(
    ...
    require(
      _toAddresses.length == _amount.length,
      'Invalid Array length, Please Try Again!!!'
    );
    uint256 total = 0;
  ...
  mapping(uint256 => mapping(address => uint256[]))
    private ticketNumberByAddress; //user ticket number for a saleId
  mapping(uint256 => mapping(uint256 => address[]))
    private addressesByTicketNumber; //list of addresses for ticketId
  mapping(uint256 => mapping(address => uint256))
    private totalSaleAmountByAddAndSaleID; //list of addresses for ticketId
  ...
  uint256 private saleIdNow = 1; //saleIdNow for sale block.timestamp
   ...
   */
  function initiateSmartContractValue(uint256 _elapseTime)
    public
    onlyOwner
    returns (bool)
  {
   ...
   */
  function purchaseTicket(uint256[] memory _ticketNumbers)
    public
    payable
    returns (bool)
  {
    require(
      (_ticketNumbers.length == 1 ||
        _ticketNumbers.length == 6 ||
        _ticketNumbers.length == 12 ||
        _ticketNumbers.length == 18 ||
        _ticketNumbers.length == 24 ||
        _ticketNumbers.length == 30 ||
        _ticketNumbers.length == 36 ||
        _ticketNumbers.length == 42 ||
        _ticketNumbers.length == 48 ||
        _ticketNumbers.length == 54 ||
        _ticketNumbers.length == 60),
      'Invalid Value'
    );
    require(
      msg.value >= getNumber(_ticketNumbers.length).mul(_ticketPrice),
    ...
    if (_ticketNumbers.length > 1) {
      if ((saleStartTimeStamp[saleIdNow] + elapsedTime) > block.timestamp) {
        ...
        totalSaleAmountByAddAndSaleID[saleIdNow][msg.sender] =
          totalSaleAmountByAddAndSaleID[saleIdNow][msg.sender] +
          msg.value;
        ...
        AllParticipantAddresses.push(msg.sender);
        totalSaleAmount[saleIdNow + 1] =
          totalSaleAmount[saleIdNow + 1] +
          msg.value;
        totalSaleAmountForAllSales = totalSaleAmountForAllSales + msg.value;
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
          ticketNumberByAddress[saleIdNow + 1][msg.sender].push(
            _ticketNumbers[i]
          );
        }
        if (_ticketNumbers.length == 60) {
  ...
  function declareWinner(
    uint256[] calldata _winningSequence,
    uint256 _powerballNumber,
    address payable[] calldata _winnerAddressArray,
    uint256[] calldata _winnerPositions,
    uint256[] calldata _winnerAmountInWei
  ) external payable onlyOwner returns (bool) {
    require(
      _winnerAddressArray.length == _winnerAmountInWei.length ||
        _winnerAmountInWei.length == _winnerPositions.length,
      'Invalid winner declaration data'
    );
   ...
   */
  function getWinningPowerballNumberBySaleId(uint256 _saleId)
    external
    view
    returns (uint256)
  {
  ...
  function withdrawETHFromContract(
    uint256 _savingsValue,
    address payable _savingsReceiver,
    uint256 _opexValue,
    address payable _opexReceiver
   ...
   */
  function getWinningNumber(uint256 _saleId)
    external
    view
   ...
   */
  function getWinningAmount(uint256 _saleId)
    external
    view
    returns (uint256[] memory)
  {
   ...
   */
  function getWinningAddress(uint256 _saleId)
    external
    view
   ...
   */
  function getAllSaleAddressesBySaleID(uint256 _saleId)
    external
    view
    returns (address[] memory)
  {
   ...
   */
  function getTotalSaleAmountBySaleID(uint256 _saleId)
    external
    view
    returns (uint256)
  {
   ...
   */
  function getParticipantCountBySaleId(uint256 _saleId)
    external
    view
    returns (uint256)
  {
   ...
   */
  function setPriceOfOneTicket(uint256 _newPrice)
    external
    onlyOwner
    returns (bool)
  {
   ...
   */
  function getticketNumberByAddress(uint256 _saleId, address _add)
    external
    view
    returns (uint256[] memory)
  {
   ...
   */
  function setpurchaseTokenAmount(uint256 purchaseTokenAmount)
    external
    onlyOwner
    returns (bool)
  {
   ...
   */
  function setbuyerPoolAddress(address buyerPoolAddress)
    external
    onlyOwner
    returns (bool)
  {
   ...
   */
  function getTotalSaleAmountByAddAndSaleID(
    uint256 _saleId,
